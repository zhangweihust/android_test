...
public class QTTS {
	...
	public void Init() {
		...
		int ret = MSC.QTTSInit(("appid=" + appID).getBytes());
	...
	private void QTTSSessionBegin() {
		...
		sessinID = MSC.QTTSSessionBegin(arg.getBytes(), sInfo);
	...
	private void QTTSTextPut() {
		...
		int ret = MSC.QTTSTextPut(sessinID, sentences.getBytes());
		if (ret == 0) {
	...
	private byte[] QTTSAudioGet() {
		...
		byte[] ret = MSC.QTTSAudioGet(sessinID, sInfo);
	...
	private void QTTSSessionEnd() {
		int ret = MSC.QTTSSessionEnd(sessinID, null);
		if (ret == 0) {
			// exec ok
			status = QTTS_STATUS_INIT;
